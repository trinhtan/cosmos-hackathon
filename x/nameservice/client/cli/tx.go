...
// GetCmdSetDescription is the CLI command for sending a SetDescription transaction
func GetCmdSetDescription(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "set-description [name] [description]",
		Short: "set the description associcated with a name that you own",
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
			inBuf := bufio.NewReader(cmd.InOrStdin())
			...
			msg := types.NewMsgSetDescription(args[0], args[1], cliCtx.GetFromAddress())
			err := msg.ValidateBasic()
			if err != nil {
				return err
			}

			// return utils.CompleteAndBroadcastCLI(txBldr, cliCtx, msgs)
			return utils.GenerateOrBroadcastMsgs(cliCtx, txBldr, []sdk.Msg{msg})
		},
	}
}
